...
function getPrevYearMap(currentYear, originList, destList) {
  // Build a map: "home|work" -> prevYearValue
  ...
  const links = allData.links
    ...
    .filter(d => {
      // same filters as filteredData except the year
      const filterMatch = currentFilter(d);
      const originMatch = originList.includes("All") || originList.includes(d.home);
      const destMatch = destList.includes("All") || destList.includes(d.work);
      ...
      if (!isAllAge && isAllIncome) {
        ...
        groupMatch = isAge; // default: age only when both 'All'
      }

      return filterMatch && originMatch && destMatch && (+d.year === prevYear) && groupMatch;
    });

  // Merge by O/D for prev year
  const prevMerged = new Map();
  links.forEach(l => {
    const key = `${l.home}|${l.work}`;
    prevMerged.set(key, (prevMerged.get(key) || 0) + l.value);
  });
  return prevMerged;
}
...
function renderSankey(linkData) {
  ...
  const totalCommuters = d3.sum(linkData, d => d.value);
  const prevMap = getPrevYearMap(currentYear, currentOrigin, currentDestination);
  ...
  svg.append("g")
    ...
    .on("mouseover", (event, d) => {
      const key = `${d.source.rawName || d.source.name}|${d.target.rawName || d.target.name}`;
      const prevVal = prevMap.get(key) || 0;
      const delta = d.value - prevVal;
      const pct = totalCommuters ? (d.value / totalCommuters * 100) : 0;
      const deltaPct = prevVal ? (delta / prevVal * 100) : null;

      const arrow = delta > 0 ? "▲" : (delta < 0 ? "▼" : "—");
      const deltaTxt = (deltaPct === null)
        ? "n/a"
        : `${arrow} ${Math.abs(deltaPct).toFixed(1)}% vs. ${currentYear - 1}`;
      ...
      tooltip.html(`
        ...
        Share of Total: ${pct.toFixed(1)}%<br>
        Change: ${deltaTxt}<br>
